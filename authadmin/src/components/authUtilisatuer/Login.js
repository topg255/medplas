...
function MedicalTouristLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Stocker dans localStorage si besoin
      ...
      // Ouvrir MF2 dans un nouvel onglet
      //const newTab = window.open("http://localhost:3020", "_blank");

      // Attendre que MF2 signale qu'il est prêt
      {/*const listener = (event) => {
        if (event.origin === "http://localhost:3020" && event.data === "READY_FOR_TOKEN") {
          ...
          window.removeEventListener("message", listener); // Nettoyage
        }
      };*/}
  ...
  const goToHome = () => {
    window.location.href = "http://localhost:3020/"; // Redirection vers "/home"
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <Link to="/forgot-password" className="text-sm font-munika text-blue-600 hover:text-blue-500">
                    Forgot password?
                  </Link>
                </div>
                <div className="relative rounded-md shadow-sm">
