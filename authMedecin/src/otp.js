...
const OtpVerification = () => {
  ...
  const verifyOtp = async (otpCode) => {
     ...
     // Correction : force le rôle à "doctor" pour l'API
     const apiRole = "pdoctor";
     console.log("userRole for verify-otp:", userRole, "apiRole used:", apiRole);

     try {
       const accessToken = localStorage.getItem("accessToken");
       const apiUrl = `http://localhost:3001/users/${apiRole}/verify-otp`;
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const apiUrl = `http://localhost:3001/users/${userRole}/resend-otp`;
