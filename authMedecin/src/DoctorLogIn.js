...
function Login() {
  ...
  // Gérer le retour du navigateur
  useEffect(() => {
    const handlePopState = (event) => {
      // Empêcher le comportement par défaut du retour
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("userHospital", user.ClinicAffiliation);

      // Mettre à jour le state Redux
      dispatch(setUser(user));

      // DEBUG: Vérifier les valeurs avant redirection
      console.log("Login successful, user role:", user.role);
      console.log("Navigation vers /dashboard");

      // Redirection
      navigate('/dashboard', { replace: true }); // 'replace: true' empêche de revenir à la page de login avec le bouton retour

    } catch (error) {
      console.error("Login error:", error);
      setError(error.message || "An error occurred during login");
      // Vider le localStorage en cas d'erreur
  ...
  const goToHome = () => {
    // Rediriger vers la landing page au lieu d'utiliser navigate
    window.location.href = 'http://localhost:3020';
  };
          ...
          <div className="absolute top-6 left-6 z-30">
            <div className="flex items-center cursor-pointer" onClick={goToHome}>
              <div className="w-10 h-10 bg-transparent rounded-lg flex items-center justify-center shadow-md hover:bg-gray-700 transition duration-300">
                <FaHospital className="h-5 w-5 text-white" /> {/* Icône de formulaire */}
              </div>
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 font-munika mt-2">
                Sign in to manage your doctor compte
              </p>
            </div>
