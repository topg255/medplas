...
const RegisterClinic = () => {
    ...
    useEffect(() => {
        ...
        window.history.pushState({ page: 'clinic-login' }, '', window.location.pathname);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                throw new Error(err.message || "Clinic login failed");
            }

            const data = await resp.json();
            const { accessToken, refreshToken, user } = data;

            // Update these storage keys to match what ClinicDashboard expects
            ...
            // Clear all storage items on error
